...
gba <- fread(paste0(map_data_source,"GBA_NIETGA/GBAPERSOON2021.csv"), sep=";",
             select = c(RINPERSOONS = "character", RINPERSOON = "character", GBAGEBOORTEJAAR = "integer", GBAGESLACHT = "integer", 
                        GBAGEBOORTELAND = "integer"))
gba[, RINP := paste0(RINPERSOONS, RINPERSOON)]
setnames(gba, 3, "birthyear")
gba[, female := (GBAGESLACHT == 2)]
gba[, birthcountry := GBAGEBOORTELAND]
gba[, c("RINPERSOONS", "RINPERSOON", "GBAGESLACHT", "GBAGEBOORTELAND") := NULL]

nietgba <- fread(paste0(map_data_source,"GBA_NIETGA/NIET_GBAPERSOON.csv"), sep=";", 
                 select = c(RINPERSOONS = "character", RINPERSOON = "character", NIETGBAGEBJAAR = "integer", NIETGBAGESLACHT = "integer",
                            NIETGBAGEBLAND = "character"))
nietgba[, RINP := paste0(RINPERSOONS, RINPERSOON)]
nietgba[, birthyear := as.integer(NIETGBAGEBJAAR)]
nietgba[, female := (NIETGBAGESLACHT == 2)]
nietgba[, birthcountry := fifelse(NIETGBAGEBLAND=="----", NA_real_, as.integer(NIETGBAGEBLAND))]
nietgba[, c("RINPERSOONS", "RINPERSOON", "NIETGBAGEBJAAR", "NIETGBAGESLACHT", "NIETGBAGEBLAND") := NULL]
## remove rows with missing values, ignoring birthcountry 
nietgba <- nietgba[complete.cases(nietgba[, !c("birthcountry")]), ]
...
gba[!is.na(birthcountry.x), birthcountry := birthcountry.x]
gba[is.na(birthcountry.x) & !is.na(birthcountry.y), birthcountry := birthcountry.y]
gba[is.na(birthcountry.x) & is.na(birthcountry.y), birthcountry := NA]


gba[, c("birthyear.x", "female.x", "birthyear.y", "female.y", "birthcountry.x", "birthcountry.y") := NULL]
